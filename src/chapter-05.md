...
你已经看过这些示例了. `Feature`选项的形式是`feature = "name-of-feature"`, 如果指定的特征被启用, 该条件就会被视为为真. 你可以通过组合器在一个条件中检查多个`feature`. 例如, `any(feature = "f1", feature = "f2")`表示只要功能`f1`或`f2`中有一个被启用, 该条件就为真。
